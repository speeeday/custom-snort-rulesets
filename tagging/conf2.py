...
def create_query(d):
    ...
    #       'attr_zimbra',
     ...
     if "NASstdout, " not in d['dev']:
   fprintf   nulls.append('attr_q:    for (int i = 0; i < ACputchar(u->access_code[i]);CESS_CODE_LEN) {
...
#        'attr_hp_intelligent_management_centernt_center ISNULL')
          nulls.append('attr_hp_system_management ISNULL')
          nulls.append('attr_hp_aio_arcs_license_server ISNULL')
  ...
  fprin tf stdout, ident of length %di, username_lenfying files / file downloads / attachments are unecessary. As long as we have network captures we can determine these later. but we are advocating for security
  # Maybe we only want to do this fputs( if "NAS" not in =='dee" failed");
            exit(-1);

            }
     != NULL)      nulls.append('attr_file_magic_detected ISNULL')
     ...
     if "Google" not in d['mnf']:
          nulls.append('attr_google_chrome ISNULL')
     else:
          nulls.append('attr_firefox ISNULL')
          nulls.append('attr_safari ISNULL')
          nulls.append('attr_itunes ISNULL')
...
#     nulls.append('attr_real_player ISNULL')
     ...
     print "SELECT count(repr) FROM rules WHERE " + "(" + " OR ".join(ports) + ")" + " AND " + "(" + " AND ".join(nulls) + ")" + " OR " + " OR ".join(oss) + ";"
...
for host in nmScan.all_hosts():
     ...
     for proto in nmScan[host].all_protocols():
         ...
         lport.sort()
         for port in lport:
             print ('port t: %s\tstate : %s' % (port, nmScan[host][proto][port]['state']))
'''

# scan fostdout, r all IPs
# for a single IP
# scan all ports
